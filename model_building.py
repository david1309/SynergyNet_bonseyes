...
class I2P(nn.Module):
	...
	def forward(self,input, target):
		...
		_3D_attr_GT = target.type(torch.FloatTensor).to(self.args.device)
...
class SynergyNet(nn.Module):
	def __init__(self, args):
		...
		self.triangles = sio.loadmat('./train/SynergyNet/3dmm_data/tri.mat')['tri'] -1
		if args.use_cuda:
		...
		self.register_buffer('param_mean', torch.Tensor(param_pack.param_mean).to(args.device))
		self.register_buffer('param_std', torch.Tensor(param_pack.param_std).to(args.device))
		self.register_buffer('w_shp', torch.Tensor(param_pack.w_shp).to(args.device))
		self.register_buffer('u', torch.Tensor(param_pack.u).to(args.device))
		self.register_buffer('w_exp', torch.Tensor(param_pack.w_exp).to(args.device))

		# If doing only offline evaluation, use these
		# self.u_base = torch.Tensor(param_pack.u_base).to(args.device)
		# self.w_shp_base = torch.Tensor(param_pack.w_shp_base).to(args.device)
		# self.w_exp_base = torch.Tensor(param_pack.w_exp_base).to(args.device)

		# Online training needs these to parallel
		self.register_buffer('u_base', torch.Tensor(param_pack.u_base).to(args.device))
		self.register_buffer('w_shp_base', torch.Tensor(param_pack.w_shp_base).to(args.device))
		self.register_buffer('w_exp_base', torch.Tensor(param_pack.w_exp_base).to(args.device))
